...
(worker-expr
 '(defworker append-to-key
    {:outputKeys [:updated]}
    [{:keys [key value]}]
    (let [orig (wcar* (car/get key))
          updated (str orig value)]
      (wcar* (car/set key updated)))
    {:updated updated}))
...
;; Now we need to send the workflow to Conductor. Because both the
;; test and the worker deployment start at the same time, we have no
;; guarantee that the tasks defined by the worker deployment be
;; defined before we define the workflow. Having the workflow defiend
;; first will cause it to fail. Therefore, we define it from within a
;; loop, in which when it fails it is retried. Termination of this
;; loop is evidence for things working.
(test-expr
 '(loop []
    (if (try
...
;; Once the workflow is defined, we can trigger it.
(test-expr
 '(def wf-id (con/trigger-workflow :test-workflow 1 {})))
